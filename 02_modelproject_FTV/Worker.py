...
class WorkerClass:
    ...
    # ------------------------------------------------------------------
    # NEW: Minimum ell for positive consumption under top tax (piecewise)
    # ------------------------------------------------------------------
    def get_min_ell_top(self, p, region):
        ...
        if region == 'before':
            # c = (1-tau) w p ell - zeta
            denom = (1 - par.tau) * w * p
            if denom <= 0:
                return np.inf
            ell_min = par.zeta / denom
            return max(ell_min, 0.0) + 1e-10

        elif region == 'after':
            # for ell>=ell_k: c = (1-tau-omega) w p ell + omega*kappa - zeta
            denom = (1 - par.tau - par.omega) * w * p
            if denom <= 0:
                return np.inf
            ell_min = (par.zeta - par.omega * par.kappa) / denom
    ...
    # ------------------------------------------------------------------
    # NEW: Safe bisection root finder (checks sign change)
    # ------------------------------------------------------------------
    ...
    def optimal_choice_top(self, p):
        ...
        # IMPORTANT: use top-tax positivity bound (most robust)
        ell_min = 0.0
        # if zeta>0, ensure c>0 at least in the "before" region
    ...
    def optimal_choice_top_FOC(self, p):
        ...
        # ---------- Candidate: ell = 0 (often relevant when zeta <= 0) ----------
        U0 = -np.inf
        ell0 = 0.0

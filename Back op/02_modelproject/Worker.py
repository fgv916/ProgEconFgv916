...
class WorkerClass:
    ...
    #     worker.par.tau_star  = 0.1367
    #     worker.par.zeta_star = -0.523
    # ------------------------------------------------------------------

    # ------------------------------------------------------------------
    # Post tax income under top tax
    # ------------------------------------------------------------------
    def post_tax_income_top(self, p, ell):
        ...
        tax = par.tau_star * pre_tax + par.omega * top_base + par.zeta_star
        return pre_tax - tax


    # ------------------------------------------------------------------
    # FOC before and after the kink
    # ------------------------------------------------------------------
    def FOC_top_before(self, p, ell):
        ...
        if np.any(c <= 0):
            return 1e6
        return (1 - par.tau_star) * par.w * p / c - par.nu * ell**par.epsilon
    ...
    def FOC_top_after(self, p, ell):
        ...
        if np.any(c <= 0):
            return 1e6
        return (1 - par.tau_star - par.omega) * par.w * p / c - par.nu * ell**par.epsilon


    # ------------------------------------------------------------------
    # Minimum ell for positive consumption under top tax (piecewise)
    # ------------------------------------------------------------------
    def get_min_ell_top(self, p, region):
        """
        Minimum ell to ensure c>0 in the given region.
        region: 'before' (ell < ell_k) or 'after' (ell >= ell_k)
        """
        par = self.par
        w = par.w
        ell_k = par.kappa / (w * p)
        ...
        if region == 'before':
            ...
            denom = (1 - par.tau_star) * w * p
            if denom <= 0:
                return np.inf
            ell_min = par.zeta_star / denom
            return max(ell_min, 0.0) + 1e-10

        elif region == 'after':
            # for ell>=ell_k: c = (1-tau_star-omega) w p ell + omega*kappa - zeta_star
            denom = (1 - par.tau_star - par.omega) * w * p
            if denom <= 0:
                return np.inf
            ell_min = (par.zeta_star - par.omega * par.kappa) / denom
            return max(ell_k, ell_min, 0.0) + 1e-10

        else:
            raise ValueError("region must be 'before' or 'after'")


    # ------------------------------------------------------------------
    # Safe bisection root finder (checks sign change)
    # ------------------------------------------------------------------
    def safe_bisect_root(self, f, a, b):
        fa = f(a)
        fb = f(b)
        ...
        if fb == 0:
            return b
        if fa * fb > 0:
            return None  # no sign change => bisection invalid
    ...
    # ------------------------------------------------------------------
    # Optimal choice with top tax (direct maximization)
    # ------------------------------------------------------------------
    def optimal_choice_top(self, p):
        ...
        ell_min = 0.0
        ell_min = max(ell_min, self.get_min_ell_top(p, region="before"))
    ...
    def optimal_choice_top_FOC(self, p):
        ...
        U0 = -np.inf
        ell0 = 0.0
        c0 = self.post_tax_income_top(p, ell0)
        if c0 > 0:
            U0 = self.utility(c0, ell0)

        # ---------- STEP 1: root before kink ----------
        Ub, ell_b = -np.inf, None
        upper_1 = min(ell_k - eps, ell_max)

        ell_min_before = self.get_min_ell_top(p, region="before")
        if ell_min_before < upper_1:
            f_before = lambda ell: self.FOC_top_before(p, ell)
            root_b = self.safe_bisect_root(f_before, ell_min_before, upper_1)
            if root_b is not None:
                ell_b = root_b
                cb = self.post_tax_income_top(p, ell_b)
                if cb > 0:
                    Ub = self.utility(cb, ell_b)

        # ---------- STEP 2: kink ----------
        Uk, ell_k_feasible = -np.inf, None
        if 0.0 <= ell_k <= ell_max:
            ck = self.post_tax_income_top(p, ell_k)
            if ck > 0:
                Uk = self.utility(ck, ell_k)
                ell_k_feasible = ell_k

        # ---------- STEP 3: root after kink ----------
        Ua, ell_a = -np.inf, None

        lower_3 = max(ell_k + eps, 0.0)
        ell_min_after = self.get_min_ell_top(p, region="after")
        lower_3 = max(lower_3, ell_min_after)

        if lower_3 < ell_max:
            f_after = lambda ell: self.FOC_top_after(p, ell)
            root_a = self.safe_bisect_root(f_after, lower_3, ell_max)
            if root_a is not None:
                ell_a = root_a
                ca = self.post_tax_income_top(p, ell_a)
                if ca > 0:
                    Ua = self.utility(ca, ell_a)

        # ---------- Candidate: ell = ell_max ----------
        Umax = -np.inf
        cmax = self.post_tax_income_top(p, ell_max)
        if cmax > 0:
            Umax = self.utility(cmax, ell_max)

        # ---------- STEP 4: choose best ----------
        U_candidates   = [Ub, Uk, Ua, U0, Umax]
        ell_candidates = [ell_b, ell_k_feasible, ell_a, ell0, ell_max]
        labels         = ['b', 'k', 'a', '0', 'max']
        ...
        if (ell_star is None) or (not np.isfinite(U_star)):
            ell_star = ell_max
